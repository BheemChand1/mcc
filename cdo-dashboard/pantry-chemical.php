...
$paramsStmt = $pdo->prepare("
    ...
    WHERE m.station_id = :station_id AND m.chemical_type_id = 3 AND m.status = 'Active'
    ORDER BY p.id ASC
");
...
$stmt = $pdo->prepare("
    ...
    WHERE report_date BETWEEN :from_date AND :to_date AND station_id = :station_id AND chemical_type_id = 3
...
if (!empty($tokensList)) {
    ...
    $coachesStmt = $pdo->prepare("
        ...
        FROM mcc_chemical_report 
        WHERE token_id = :token_id AND station_id = :station_id AND chemical_type_id = 3
    ");
...
$pageTitle = 'Pantry Car Chemical Report | CDO';
    ...
    <div class="app-content">
        <div class="container-fluid">
            <form class="report-filter no-print" method="GET">
                ...
                <a href="pantry-chemical-summary.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary">Summary</a>
                <a href="pantry-chemical-target.php" class="btn-summary" style="background: #10b981 !important; margin-left: 8px;">Targets</a>
                ...
                <?php foreach ($sheetsData as $sheet): ?>
                    ...
                    <div class="chemical-sheet">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 15px;">
                            <h2 style="font-size: 18px; font-weight: 700; color: #1e293b; margin: 0;">Daily Pantry Car Chemical Report</h2>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php elseif (!empty($isCDO) && empty($sheet['is_fallback'])): ?>
                                    <button type="button" class="btn btn-sm btn-success no-print" onclick="approveReport(this, 'mcc_intensive_pantry_chemical_report', '<?= htmlspecialchars($sheet['token_id']) ?>')" style="font-weight: 600; padding: 5px 14px; border-radius: 6px; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 6px rgba(16,185,129,0.25);">
