...
// Handle POST request to update targets - Intensive Chemical
    ...
    if (!empty($isViewer)) {
        ...
        try {
            ...
            $findActiveStmt = $pdo->prepare("
                ...
                WHERE station_id = :station_id AND chemical_type_id = 2 AND parameter_id = :parameter_id AND effective_to IS NULL
                LIMIT 1
            ");
            ...
            $insertNewStmt = $pdo->prepare("
                ...
                VALUES (:parameter_id, 2, :station_id, :qty, :penalty, :penalty_qty, :effective_from, NULL)
            ");
            ...
            $pdo->commit();
            $successMessage = 'Intensive chemical targets updated successfully.';
...
// Fetch active chemical parameters for the station - Intensive
$paramsStmt = $pdo->prepare("
    ...
    WHERE m.station_id = :station_id AND m.chemical_type_id = 2 AND m.status = 'Active'
    ORDER BY p.id ASC
");
$paramsStmt->execute(['station_id' => $stationId]);
$parametersList = $paramsStmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch current active targets - Intensive
$targetsStmt = $pdo->prepare("
    SELECT t.* 
    FROM mcc_chemical_target t
    WHERE t.station_id = :station_id AND t.chemical_type_id = 2 AND t.effective_to IS NULL
");
...
$pageTitle = 'Manage Intensive Chemical Targets | CDO';
    ...
    <div class="app-content">
        <div class="container-fluid" style="padding-top: 20px;">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 style="font-size: 20px; font-weight: 700; margin: 0; color: #1e293b;">Manage Intensive Chemical Targets & Penalties</h1>
                <a href="intensive-chemical-report.php" class="btn btn-secondary" style="background: #64748b; border: none; font-weight: 600;">
            ...
            <div class="card shadow-sm border-0 mb-4" style="border-radius: 12px; overflow: hidden;">
                <div class="card-header border-0 py-3">
                    <h5 class="card-title mb-0" style="font-size: 16px;"><i class="bi bi-gear-fill me-2"></i>Set Target Rates & Deficit Penalties (Intensive)</h5>
                </div>
