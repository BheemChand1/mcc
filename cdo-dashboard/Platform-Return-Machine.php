...
// Fetch targets that overlap the selected date range - PRT
...
// Fetch all daily reports in the range, keeping each day's shift values separate - PRT
    ...
    <div class="app-content">
        <div class="container-fluid">
            <form class="report-filter no-print" method="GET">
                ...
                <a href="Platform-Return-Machine-target.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary" target="_blank">Machine Target</a>
                <a href="Platform-Return-Machine-summary.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary">Summary</a>
            ...
            <div class="report-wrap">
                ...
                <div class="report-frame">
                    <div class="report-header" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #999; padding-bottom: 10px; margin-bottom: 15px;">
                        <h2 style="margin: 0;">Daily Machine Report (Platform Return)</h2>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <?php elseif (!empty($isCDO)): ?>
                                <button type="button" class="btn btn-sm btn-success no-print" onclick="approveReport(this, 'mcc_prt_machine_report', '', {report_date: '<?= htmlspecialchars($sheet['report_date']) ?>'})" style="font-weight: 600; padding: 5px 14px; border-radius: 6px; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 6px rgba(16,185,129,0.25);">
