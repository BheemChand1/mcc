...
// Fetch distinct tokens/trains in this date range for intensive report
$stmt = $pdo->prepare("
...
// Fetch active parameters and subparameters from the database for intensive report
...
$pageTitle = 'Intensive Score Card | MCC';
    ...
    <div class="app-content">
        <div class="container-fluid">
            <form class="report-filter no-print" method="GET">
                ...
                <a href="intensive-summary.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary" target="_blank">Summary</a>
                ...
                <?php foreach ($sheets as $sheet): ?>
                    <div class="report-frame">
                        <div class="report-header" style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #999; padding-bottom: 10px; margin-bottom: 15px;">
                            <h2 style="margin: 0;">Score Card For Intensive Cleaning</h2>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php elseif (!empty($isCDO) && empty($sheet['is_fallback'])): ?>
                                    <button type="button" class="btn btn-sm btn-success no-print" onclick="approveReport(this, 'mcc_intensive_scorecard_report', '<?= htmlspecialchars($sheet['token_id']) ?>')" style="font-weight: 600; padding: 5px 14px; border-radius: 6px; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 6px rgba(16,185,129,0.25);">
