...
try {
    // 1. Fetch scores joined with parameter details for the specific coach - PRT
    $stmt = $pdo->prepare("
        SELECT 
            ...
            r.auditor_name,
            r.report_date
        FROM mcc_prt_scorecard_report r
        JOIN mcc_prt_scorecard_sub_param sp ON r.sub_parameter_id = sp.id
    ...
    // 2. Fetch rating groups and options dynamically - PRT
    $ratingsStmt = $pdo->query("SELECT rating_group, rating_name, rating_value FROM mcc_prt_rating ORDER BY id ASC");
