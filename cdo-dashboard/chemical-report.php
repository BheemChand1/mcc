...
$stmt = $pdo->prepare("
    ...
    WHERE report_date BETWEEN :from_date AND :to_date AND station_id = :station_id AND chemical_type_id = 1
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 15px;">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php elseif (!empty($isCDO) && empty($sheet['is_fallback'])): ?>
                                    <button type="button" class="btn btn-sm btn-success no-print" onclick="approveReport(this, 'mcc_normal_chemical_report', '<?= htmlspecialchars($sheet['token_id']) ?>')" style="font-weight: 600; padding: 5px 14px; border-radius: 6px; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 6px rgba(16,185,129,0.25);">
