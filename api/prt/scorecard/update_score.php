...
try {
    // Verify if the scorecard record exists - PRT
    $checkStmt = $pdo->prepare("
        SELECT COUNT(*) FROM mcc_prt_scorecard_report 
    ...
    if ($checkStmt->fetchColumn() == 0) {
        ...
        echo json_encode([
            "status" => "error",
            "message" => "Scorecard record not found for token_id: $tokenId, sub_parameter_id: $subParameterId, coach_no: $coachNo"
        ]);
        exit();
    }

    // Update the score value - PRT
    ...
    echo json_encode([
        "status" => "success",
        "message" => "Score value updated successfully."
    ]);
