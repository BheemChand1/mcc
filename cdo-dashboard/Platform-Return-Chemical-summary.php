...
// Fetch chemical parameters and target values for this station - PRT
...
foreach ($tokensList as $t) {
    ...
    foreach ($parametersList as $p) {
        $pId = $p['parameter_id'];
        $targetMonthQty = floatval($p['qty_ml'] ?? 0);
        $dailyTarget = $targetMonthQty / 30.0;
        $consumedTotal = $tokenParamQty[$pId] ?? 0.0;

        if (isset($monthlyParamData[$pId])) {
            $monthlyParamData[$pId]['monthly_target'] += $dailyTarget;
        }

        if ($dailyTarget > 0) {
            $compliance = min(100.0, ($consumedTotal / $dailyTarget) * 100.0);
            $paramCompliances[] = $compliance;
        } else {
            $paramCompliances[] = 100.0;
        }

        if ($consumedTotal < $dailyTarget) {
            $deficit = $dailyTarget - $consumedTotal;
...
// Compute aggregate monthly score
$totalAchieved = 0.0;
...
$pageTitle = 'Monthly Chemical Summary Report (PRT) | MCC';
        ...
        <div class="container-fluid" style="padding-top: 15px;">
            ...
            <div class="report-sheet-frame">
                ...
                <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 8px 18px; font-size: 13px; font-weight: bold; border-top: 1.5px solid #000; border-bottom: 1.5px solid #000; padding: 8px 0; margin-bottom: 20px;">
                    ...
                    <div>
                        Depot : <span class="underlined-value"><?= htmlspecialchars($stationName) ?></span>
                    </div>
                    <div>
                    ...
                    <table class="report-table">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="2" style="width: 100px;">Target</th>
                                <th rowspan="2" style="width: 80px;">Units</th>
                                <th rowspan="2" style="width: 120px; white-space: nowrap;">Penalty</th>
                        ...
                        <tbody>
                            <?php 
                             ...
                             ?>
                                <tr>
                                    <td><?= $serial++ ?></td>
                                    <td class="text-left" style="text-align: left;"><?= htmlspecialchars($data['name']) ?></td>
                                    <td><?= number_format($target, 2) ?></td>
                                    <td><?= htmlspecialchars($data['units']) ?></td>
                                    <td style="white-space: nowrap; font-size: 11.5px;">
                                        <?= $data['penalty_rate'] > 0 ? 'Rs.' . number_format($data['penalty_rate'], 0) . '/' . number_format(($data['penalty_qty_ml'] > 0 ? $data['penalty_qty_ml'] : 1), 0) . ' ' . htmlspecialchars($data['units']) : '0' ?>
                                    </td>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($shiftsList as $shift): ?>
                                            <td><?= number_format($data['shift_qtys'][$shift['shift_id']] ?? 0, 2) ?></td>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                    
                                    <td><strong><?= number_format($totalQty, 2) ?></strong></td>
                                    <td><?= ($difference > 0 ? '+' : '') . number_format($difference, 2) ?></td>
