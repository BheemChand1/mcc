...
try {
    ...
    // 2. Fetch targets/nominations active on this report date (supporting shift_id filter if provided)
    $targetQuery = "
    ...
    if (empty($tokenId)) {
        $tokenId = 'TKN-DC-MCH-' . date('Ymd', strtotime($reportDate)) . '-' . ($shiftId ?? 0) . '-' . rand(1000, 9999);
    }
