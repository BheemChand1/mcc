...
$stmt = $pdo->prepare("
    ...
    WHERE report_date BETWEEN :from_date AND :to_date AND station_id = :station_id AND chemical_type_id = 5
...
if (!empty($tokensList)) {
    ...
    $coachesStmt = $pdo->prepare("
        ...
        FROM mcc_chemical_report 
        WHERE token_id = :token_id AND station_id = :station_id AND chemical_type_id = 5
    ");
...
$pageTitle = 'Vande Bharat Chemical Report | MCC';
...
<main class="app-main">
    <div class="app-content">
        <div class="container-fluid">
            <form class="report-filter no-print" method="GET" action="vande-bharat-chemical.php">
                ...
                <a href="vande-bharat-chemical-summary.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary">Summary</a>
                <a href="vande-bharat-chemical-target.php" class="btn-summary" style="background: #10b981 !important; margin-left: 8px;">Targets</a>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 15px;">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php elseif (!empty($isCDO) && empty($sheet['is_fallback'])): ?>
                                    <button type="button" class="btn btn-sm btn-success no-print" onclick="approveReport(this, 'mcc_vb_chemical_report', '<?= htmlspecialchars($sheet['token_id']) ?>')" style="font-weight: 600; padding: 5px 14px; border-radius: 6px; display: inline-flex; align-items: center; gap: 5px; box-shadow: 0 2px 6px rgba(16,185,129,0.25);">
