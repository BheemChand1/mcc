...
// 2. Fetch active parameters for PF Return Trains for this station
...
$reportsStmt = $pdo->prepare("
    ...
    WHERE r.station_id = :station_id AND r.category = 'pf_trains' AND r.report_date BETWEEN :from_date AND :to_date
...
foreach ($groupedSheets as $sheet) {
    $sheet['percentage'] = round(($sheet['total_score'] / 25) * 100, 1);
    $sheets[] = $sheet;
}
...
$pageTitle = 'PF Return Trains Surprise Visit | CDO';
    ...
    <div class="app-content">
        <div class="container-fluid">
            ...
            <form class="report-filter no-print" method="GET">
                ...
                <a href="surprise-pf-trains-summary.php?month=<?= date('m', strtotime($fromDate)) ?>&year=<?= date('Y', strtotime($fromDate)) ?>" class="btn-summary">Summary</a>
                ...
                <?php foreach ($sheets as $sheet): ?>
                    ...
                    <div class="report-frame">
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 15px;">
                            <h2 style="font-size: 18px; font-weight: 700; color: #1e293b; margin: 0;">Surprise Visit Score Card - PF Return Trains</h2>
                            ...
                            <table class="report-table table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-light font-weight-bold">
                                        ...
                                        <td class="font-weight-bold text-primary"><?= $sumScore; ?> / 25 (<?= $percentage; ?>%)</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <div class="signature-box" style="text-align: center; width: 200px; display: flex; flex-direction: column; align-items: center;">
                                <div class="signature-img-wrap" style="height: 24px; display: flex; align-items: flex-end; justify-content: center; margin-bottom: 2px;">
                                    <?php if (!empty($sheet['isApproved']) && !empty($cdoSignature) && file_exists(__DIR__ . '/uploads/signatures/' . $cdoSignature)): ?>
                                        <img src="uploads/signatures/<?= htmlspecialchars($cdoSignature) ?>" alt="Contractor Sign" style="max-height: 24px; max-width: 95px; object-fit: contain;">
                                    <?php endif; ?>
                                </div>
