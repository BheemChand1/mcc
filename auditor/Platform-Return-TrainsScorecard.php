...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_sheet_changes'])) {
    ...
    if (!empty($targetToken) && !empty($postedScores)) {
        try {
            ...
            $chk = $pdo->prepare("SELECT COUNT(*) FROM mcc_prt_scorecard_report WHERE token_id = :tok AND station_id = :sid AND audit_by = :aud");
            $chk->execute(['tok' => $targetToken, 'sid' => $stationId, 'aud' => $auditorId]);

            if ($chk->fetchColumn() > 0) {
                $updStmt = $pdo->prepare("
                    UPDATE mcc_prt_scorecard_report 
                    SET score_value = :val 
                    WHERE token_id = :tok 
                      AND station_id = :sid 
                      AND audit_by = :aud 
...
$ratingStmt = $pdo->query("SELECT rating_name, rating_value, rating_group FROM mcc_prt_rating ORDER BY rating_value DESC");
