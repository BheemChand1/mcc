...
try {
    // 1. Fetch active parameters and subparameters for pantry car scorecard
    ...
    // Fetch chemical parameters for pantry car
    $chemParamsStmt = $pdo->prepare("SELECT id FROM mcc_intensive_pantry_chemical_param WHERE station_id = :station_id");
    ...
    // 3. Insert initial scorecard records (all scores set to empty string / NULL)
    ...
    // 4. Insert initial chemical report records if chemical table & params exist
    if (!empty($chemParams)) {
    ...
    echo json_encode([
        "status" => "success",
        "message" => "Pantry car intensive cleaning scorecard sheet initialized successfully.",
        "token_id" => $tokenId,
        "train_no" => $trainNo,
