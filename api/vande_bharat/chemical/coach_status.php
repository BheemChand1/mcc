...
try {
    // Query coach completion counts in chemical reports table - Vande Bharat (type_id = 5)
    $stmt = $pdo->prepare("
        SELECT coach_no, 
               SUM(CASE WHEN qty_used IS NULL OR qty_used = '' THEN 1 ELSE 0 END) AS empty_count,
               COUNT(*) AS total_count
         FROM mcc_chemical_report
         WHERE token_id = :token_id AND train_no = :train_no AND chemical_type_id = 5
         GROUP BY coach_no
    ...
    if (empty($rows)) {
        ...
        echo json_encode([
            "status" => "error",
            "message" => "No chemical records found for the given token_id and train_no."
        ]);
        exit();
    }
