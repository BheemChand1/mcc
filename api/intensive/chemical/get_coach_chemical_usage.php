...
try {
    // 1. Get token metadata (station_id, report_date, auditor_name) - Intensive (type_id = 2)
    $metaStmt = $pdo->prepare("
        SELECT DISTINCT station_id, report_date, auditor_name 
        FROM mcc_chemical_report
        WHERE token_id = :token_id AND train_no = :train_no AND coach_no = :coach_no AND chemical_type_id = 2
        LIMIT 1
    ");
    $metaStmt->execute([
        'token_id' => $tokenId,
    ...
    // 2. Fetch active chemical parameters for the station - Intensive (type_id = 2)
    ...
    // 3. Fetch targets active on this report date (SCD Type 2 unique bindings to prevent HY093) - Intensive (type_id = 2)
    ...
    // 4. Fetch currently saved quantities - Intensive (type_id = 2)
