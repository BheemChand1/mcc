...
try {
    ...
    // Fetch chemical parameters for the station - Intensive (type_id = 2)
    $chemParamsStmt = $pdo->prepare("
        ...
        WHERE p.station_id = :station_id AND m.chemical_type_id = 2 AND p.status = 'Active' AND m.status = 'Active'
        ORDER BY p.id ASC
    ");
    ...
    $insertStmt = $pdo->prepare("
        ...
        (sub_parameter_id, station_id, token_id, train_no, coach_no, score_value, submitted_by, audit_by, report_date)
        VALUES (:sub_parameter_id, :station_id, :token_id, :train_no, :coach_no, '', :submitted_by, :audit_by, :report_date)
    ");
    ...
    if (!empty($chemParams)) {
        $insertChemStmt = $pdo->prepare("
            ...
            (chemical_type_id, parameter_id, coach_no, qty_used, auditor_name, station_id, token_id, train_no, audit_by, report_date)
            VALUES (2, :parameter_id, :coach_no, NULL, :auditor_name, :station_id, :token_id, :train_no, :audit_by, :report_date)
        ");
